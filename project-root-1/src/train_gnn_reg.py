...
import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"   # ✅ OpenMP fix for macOS
import matplotlib.pyplot as plt
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score
from torch_geometric.loader import DataLoader
from torch_geometric.nn import GCNConv, global_mean_pool, BatchNorm
from rdkit import Chem
from utils import mol_to_graph, scaffold_split, set_seed  # ✅ reproducibility import
...
# --- Reproducibility ---
SEED = 42
g = set_seed(SEED)   # fixes numpy, torch, python RNGs
...
EPOCHS, patience, best_loss, patience_counter = 200, 20, float("inf"), 0
...
for ep in range(EPOCHS):
    ...
    if avg_val < best_loss:
        best_loss = avg_val
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(models_dir, "best_gnn_reg.pt"))
    else:
        patience_counter += 1
...
metrics = {"GNN_Reg": {"RMSE": rmse, "MAE": mae, "R2": r2}}

# --- Save metrics ---
pd.DataFrame(metrics).T.to_csv(os.path.join(metrics_dir, "train_gnn_regression_metrics.csv"))
...
plt.title("GNN Regression Training Curve")
plt.savefig(os.path.join(plots_dir, "gnn_reg_training_curve.png"))
plt.close()
...
plt.title("GNN Regression: True vs Predicted")
plt.savefig(os.path.join(plots_dir, "train_gnn_reg_scatter.png"))
plt.close()

print("✅ GNN regression training finished. Model, metrics, and plots saved.")
